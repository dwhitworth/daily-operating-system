...
**Writing style:** Before writing prose meant to be read or pasted elsewhere (the Executive Summary, and especially the 1:1 Items Discussed / Action Items artefact), read `.pi/skills/unslop-writing/SKILL.md` and apply it — no AI-default phrasing, no em-dashes. The terse internal bullets (timeline, TODOs, tags) don't need it.

**🔴 Length and density:** read `.pi/skills/terse/SKILL.md` and apply it to the journal entry, the board rebuild and everything said in chat. Hard caps live there.
...
| `meetings-[date].md` | **full texture of every meeting that ran** — who said what, verbatim fragments, outcomes, what went unraised | **the operator** (the surface actually read day to day) + the wrap |
