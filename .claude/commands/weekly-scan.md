...
2. **Scan all `Next Catchup Agenda` blocks.** ⚠️ **Do NOT read all ~106 people files whole — that's ~120k tokens and it will force a compaction.** Slice the section instead:
   ...
   - **🔴 Every `🅾️ OWED` line must carry a size (`5m` / `30m` / `block`) and a `since DD/MM`.** Missing either is a finding, and the fix is to add them in the same pass. Without `size` the debt cannot be ranked against the calendar; without `since` it cannot age. *(Added 09/09: the by-when contract had existed for weeks and not one OWED line carried one, which is exactly why debts were being discovered at prep.)*
   - **Report every OWED item older than 14 days by its `since` date as due for promotion into `## Today`**, independent of when the next 1:1 falls. This is the only mechanism covering debts to people with no recurring slot: peer managers, stakeholders, projects.
